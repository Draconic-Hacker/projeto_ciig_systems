    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-lg p-6">
      ...
      <div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-4 h-64 overflow-y-auto mb-4">
        <div v-for="(msg, index) in messages" :key="index" :class="msg.type === 'user' ? 'text-right' : 'text-left'">
          <p class="text-sm mb-2" :class="msg.type === 'user' ? 'text-blue-500' : 'text-gray-300'">{{ msg.text }}</p>
        </div>
      </div>
...
const messages = ref([
  { text: 'Olá! Sou o assistente virtual do CIIG System. Como posso ajudar você hoje?', type: 'bot' }
])
...
const sendMessage = () => {
  if (input.value.trim()) {
    ...
    setTimeout(() => {
      messages.value.push({
        text: 'Obrigado por sua pergunta. Nossa equipe responderá em breve!',
        type: 'bot'
      })
    }, 800)
